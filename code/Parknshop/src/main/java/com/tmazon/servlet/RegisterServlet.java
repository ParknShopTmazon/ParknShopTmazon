...
public class RegisterServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		req.getRequestDispatcher("WEB-INF/customer/register.jsp").forward(req,
				resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (!User.ROLE_CUSTOMER.equals(role)
				&& !User.ROLE_SHOP_OWNER.equals(role)) {
			req.setAttribute(AttrName.RequestScope.ERROR_PARAM_ROLE, true);
			req.getRequestDispatcher("WEB-INF/customer/register.jsp").forward(
					req, resp);
			return;
		}
		User user = new User(null, name, password, role, User.STATUS_NORMAL);
